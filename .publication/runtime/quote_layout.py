...
def extract(source,fmt):
    ...
    for n,row in enumerate(rows,1):
        ...
        boundary_errors=callout_structure.matn_errors([{'text':p['text'],'depth':p.get('quote_depth',0)} for p in paragraphs])
        if boundary_errors and paragraphs:paragraphs[0]['cues'].append('isnad-matn-layout')
