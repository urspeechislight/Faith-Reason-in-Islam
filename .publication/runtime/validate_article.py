...
def _check(src, register='standard'):
    ...
    for am in re.finditer(r'<article[^>]*>.*?</article>', src, re.S):
        ...
        bqs=[n for n in art_index.nodes if n['tag']=='blockquote' and not set(n['attrs'].get('class','').split()) & {'source-speech','source-matn'}]
    ...
    for sm in scopes:
        ...
        for quote in sorted(scope_index.nodes,key=lambda n:n['start']):
            if quote['tag']!='blockquote' or set(quote['attrs'].get('class','').split()) & {'source-speech','source-matn'}:continue
            b = quote['start']
    ...
    for node in index.nodes:
        ...
        if classes & {'source-speech','source-matn'}:
            parents=[n for n in index.nodes if n['tag']=='blockquote' and n['start']<node['start'] and n['end']>node['end']]
            if (len(parents)!=1 or not set(parents[0]['attrs'].get('class','').split()) & {'hadith-callout','quran-callout'} or node['attrs'].get('data-quote-role') not in ('speech','matn')):
                errs.append('nested quoted body requires one source callout parent and its explicit role')
        if kind:
    ...
    for kind, node, body in callouts:
        ...
        # Every scripture exhibit has original, romanization and direct English.
        biblical_original = any(n['attrs'].get('lang') and n['attrs'].get('lang')!='ar' and not {'translation','transliteration'}.intersection(n['attrs'].get('class','').split()) for n in pnodes)
        ...
        if kind == 'quran-callout' and not translits:
    ...
    _shared = Path(__file__).resolve().parent/'scripture.py'
    if not _shared.exists():_shared=Path.home()/'.agents/prose/scripture.py'
    import importlib.util as _ilu
    _sp=_ilu.spec_from_file_location('article_scripture',_shared)
    _sc=_ilu.module_from_spec(_sp);_sp.loader.exec_module(_sc)
    errs.extend(_sc.errors(src,'html'))
    return errs
...
FIXTURES = [
    ...
    ('bible original without romanization fails', _mini('<blockquote class="quran-callout"><p lang="grc">λογος</p><p class="translation">Word.</p><cite>John 1:1, Textus Receptus</cite></blockquote>'), 'carries no transliteration', None),
