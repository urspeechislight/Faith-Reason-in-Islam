...
REPO = Path(os.environ["PUBLICATION_SITE_DIR"])
LEGACY = json.loads((Path(__file__).resolve().parent / "legacy.json").read_text())["files"]
...
DOMAIN_MAP = {
    "quran": ["quran-preservation", "quran-contradictions", "borrowing",
              ...
              "allah-prays", "quran-we-he-i", "sister-of-aaron"],
...
BLURB = {
    ...
    "quran-we-he-i": "The Quran's We is the Arabic plural of majesty, its He is the same Speaker presenting Himself, and its I is that Speaker at His nearest, shown from the grammarians, the poets, the book's own bookkeeping, the school's tafsir, the hadith of the Ahl al-Bayt, and the Christian canon's own divine plurals.",
    ...
    "ibn-taymiyya-record": "The school that venerated him also put him on trial: the biographical dictionary, the traveler who sat in his mosque, his own collected works, and the judges of Egypt and Damascus carry the creed he taught in his own words, the grave-visitation rulings, twenty-three years of imprisonments, and the elegies after his death in the citadel.",
...
def article(slug: str) -> dict:
    ...
    legacy = hashlib.sha256(src.encode()).hexdigest() == LEGACY.get(slug+".html")
    blurb = BLURB.get(slug) if legacy else None
    if not blurb and desc:
        blurb = re.split(r"(?<=[.!?])\s", desc)[0]
    return {
        "slug": slug, "title": TITLE.get(slug, title) if legacy else title, "blurb": blurb or "",
