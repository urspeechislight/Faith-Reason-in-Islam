...
def refresh_manifests():
    for folder,manifest in [('runtime','runtime-manifest.json'),('authoring','authoring-manifest.json')]:
        paths=[p for p in (ROOT/folder).rglob('*') if p.is_file() and p.suffix in {'.py','.md','.html','.css','.js','.json'} and '__pycache__' not in p.parts]
        for path in paths:
