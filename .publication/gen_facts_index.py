...
for page in sorted(REPO.glob("*.html")):
    match = re.search(r'<main\b[^>]*data-category=["\'](debate|exegesis|narration|commentary)["\']', page.read_text())
    if match: INV[page.name] = {"cat": match[1]}

ORDER = ["debate", "exegesis", "narration", "commentary"]
CAT_LABEL = {"debate": "Debate Articles", "exegesis": "Exegesis Articles", "narration": "Narration Articles", "commentary": "Commentary Articles"}
